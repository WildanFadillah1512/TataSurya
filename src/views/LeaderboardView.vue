...
const getCurrentLabel = computed(() => {
  const allFilters = [
    ...missionGroups.planets,
    ...missionGroups.phenomena,
    ...missionGroups.games,
  ];
  const found = allFilters.find((f) => f.id === activeSubFilter.value);
  return found ? found.label : "UNKNOWN MISSION";
});
...
const getRankGradient = (rank) => {
  if (rank === 0)
    return "from-yellow-300 to-yellow-600 shadow-yellow-500/50 border-yellow-400";
  if (rank === 1)
    return "from-gray-300 to-gray-500 shadow-gray-400/50 border-gray-300";
  if (rank === 2)
    return "from-orange-400 to-amber-700 shadow-orange-500/50 border-orange-400";
  ...
  >
    ...
    >
      ...
      >
        ...
        <template v-else>
          <div
            class="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 items-end justify-center mb-4 md:mb-0 order-1 md:order-none"
          >
            <div
              v-if="topThree[1]"
              class="order-2 md:order-1 relative group perspective-1000"
            >
              <div
              ...
              ></div>
              <div
                class="relative bg-black/40 backdrop-blur-xl border border-white/10 rounded-[2rem] p-6 flex flex-col items-center text-center transform transition-transform duration-500 hover:-translate-y-2 hover:rotate-y-6 shadow-2xl overflow-hidden"
              >
                <div
                  class="absolute inset-0 bg-gradient-to-b from-transparent via-white/5 to-transparent h-[200%] w-full animate-[scan_3s_linear_infinite] pointer-events-none"
                ></div>

                <div
                  class="w-20 h-20 rounded-2xl bg-gradient-to-br from-gray-700 to-gray-900 border-2 border-gray-400 mb-4 flex items-center justify-center text-2xl shadow-[0_0_20px_rgba(156,163,175,0.4)] relative z-10"
                >
                  {{ topThree[1].name.charAt(0).toUpperCase() }}
                </div>
                <div
                  class="text-gray-300 font-black text-4xl mb-1 drop-shadow-lg"
                >
                  #2
                </div>
                <h3
                  class="text-white font-bold text-lg tracking-wide truncate w-full mb-1"
                >
                  {{ topThree[1].name }}
                </h3>
                <div
                ...
                >
                  {{ topThree[1].type || "Explorer" }}
                </div>
                <div
                  class="bg-white/5 px-4 py-2 rounded-lg border border-white/10 font-mono text-cyan-300 font-bold text-xl w-full"
                >
                  {{ topThree[1].score }}
                </div>
              </div>
            ...
            <div
              v-if="topThree[0]"
              class="order-1 md:order-2 relative group perspective-1000 z-20 md:-mt-12"
            >
              <div
              ...
              ></div>
              <div
                class="absolute -top-10 left-1/2 -translate-x-1/2 text-5xl animate-bounce drop-shadow-[0_0_15px_rgba(250,204,21,0.8)] z-30"
              >
                👑
              </div>

              <div
                class="relative bg-gradient-to-b from-yellow-900/40 to-black/60 backdrop-blur-xl border-2 border-yellow-500/50 rounded-[2.5rem] p-8 flex flex-col items-center text-center transform transition-transform duration-500 hover:-translate-y-4 shadow-[0_0_50px_rgba(234,179,8,0.2)] overflow-hidden"
              >
                <div
                  class="absolute inset-0 bg-gradient-to-tr from-transparent via-white/10 to-transparent rotate-45 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"
                ></div>

                <div
                  class="w-28 h-28 rounded-3xl bg-gradient-to-br from-yellow-600 to-yellow-900 border-2 border-yellow-300 mb-6 flex items-center justify-center text-4xl shadow-[0_0_30px_rgba(250,204,21,0.5)] relative z-10"
                >
                  {{ topThree[0].name.charAt(0).toUpperCase() }}
                </div>
                <div
                  class="text-yellow-400 font-black text-6xl mb-2 drop-shadow-[0_0_10px_rgba(250,204,21,0.6)]"
                >
                  #1
                </div>
                <h3
                  class="text-white font-black text-2xl tracking-wide truncate w-full mb-1"
                >
                  {{ topThree[0].name }}
                </h3>
                <div
                  class="text-xs text-yellow-200/60 uppercase tracking-widest mb-4"
                >
                  {{ topThree[0].type || "Commander" }}
                </div>
                <div
                  class="bg-yellow-500/20 px-6 py-3 rounded-xl border border-yellow-500/50 font-mono text-yellow-300 font-black text-3xl w-full tracking-wider shadow-[inset_0_0_20px_rgba(250,204,21,0.1)]"
                >
                  {{ topThree[0].score }}
                </div>
              </div>
            ...
            <div
              v-if="topThree[2]"
              class="order-3 md:order-3 relative group perspective-1000"
            >
              <div
              ...
              >
                <div
                  class="absolute inset-0 bg-gradient-to-b from-transparent via-white/5 to-transparent h-[200%] w-full animate-[scan_4s_linear_infinite] pointer-events-none"
                ></div>

                <div
                  class="w-20 h-20 rounded-2xl bg-gradient-to-br from-orange-700 to-orange-900 border-2 border-orange-400 mb-4 flex items-center justify-center text-2xl shadow-[0_0_20px_rgba(251,146,60,0.4)] relative z-10"
                >
                  {{ topThree[2].name.charAt(0).toUpperCase() }}
                </div>
                <div
                  class="text-orange-400 font-black text-4xl mb-1 drop-shadow-lg"
                >
                  #3
                </div>
                <h3
